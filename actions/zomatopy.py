...
class Zomato:
    ...
    def restaurant_search_by_rating(self, query="", latitude="", longitude="", cuisines="", search_offset=0):
        """
        Takes either query, latitude and longitude or cuisine as input.
        Returns a list of Restaurant IDs.
        """
        cuisines = "%2C".join(cuisines.split(","))
        headers = {'Accept': 'application/json', 'user-key': self.user_key}
        request = base_url + "search?q=" + str(query) + "&start=" + str(search_offset) + "&lat=" + str(latitude) + "&lon=" + str(longitude) + "&cuisines=" + str(cuisines) + "&sort=rating&order=desc"
        print(request)
        r = (requests.get(request, headers=headers).content).decode("utf-8")
        return r
